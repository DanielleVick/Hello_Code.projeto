...
class Fase3Screen:
    ...
    def desenhar(self, tela):
        ...
        # 2. Título
        y = self._draw_title_block(tela, self.desafio_atual["titulo"], 30, ROSA_NEON)

        # 3. Instrução bloco 1
        y += 20
        y = self._draw_text_block(tela, self.desafio_atual.get("instrucao_bloco_1", ""), y, AZUL_NEON)

        # 4. Instrução bloco 2
        y += 20
        y = self._draw_text_block(tela, self.desafio_atual.get("instrucao_bloco_2", ""), y, BRANCO)

        # 5. Lógica (se existir)
        if self.desafio_atual.get("logica"):
            y += 25
            y = self._draw_text_block(tela, self.desafio_atual["logica"], y, BRANCO)

        # 6. Pergunta
        y += 30
        y = self._draw_text_block(tela, self.desafio_atual.get("pergunta", ""), y, ROSA_NEON)
        ...
        # 8. Dica
        y += 20
        y = self._draw_text_block(tela, "Digite a resposta no campo abaixo.", y, VERDE_NEON)
