    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if self.active:
                ...
                self.txt_surface = self.font.render(self.get_display_text(), True, BRANCO)
        return None
...
class StartScreen:
    ...
    def desenhar(self, tela):
        ...
        if self.mostrar_start:
            texto_start = self.fonte_start.render("Pressione Enter para iniciar", True, BRANCO)
            tela.blit(texto_start, (self.pos_start[0] - texto_start.get_width() // 2, self.pos_start[1]))